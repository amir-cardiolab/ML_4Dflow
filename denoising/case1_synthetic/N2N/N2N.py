...
plt.imshow(X_test.reshape(m,-1).reshape(m,3,30,30,30)[150,1,:,:,15].T,origin='lower')
...
for epoch in range(num_epochs):
    ...
    for x in data_loader:
        ...
        snapshot_noisy = add_noise(snapshot.reshape(-1,n),noise_factor,noise_fraction)  #1st noisy sample
        snapshot_noisy = snapshot_noisy.reshape(-1,3,shape_x,shape_y,shape_z).to(device)#2nd noisy sample
        
        snapshot_noisy_2 = add_noise(snapshot.reshape(-1,n),noise_factor,noise_fraction)
        snapshot_noisy_2 = snapshot_noisy_2.reshape(-1,3,shape_x,shape_y,shape_z).to(device)
        recon = model(snapshot_noisy)

        loss = criterion(recon, snapshot_noisy_2)
...
#             outputs.append((epoch+1,batch_iter, snapshot))
#             del snapshot
#             outputs.append((recon))
#             del recon
#             outputs.append((snapshot_noisy))
#             del snapshot_noisy
#         else:
...
torch.save(model.state_dict(),"./N2N_3Dconv_net" + ".pt")
...
x_out_shuffled = torch.cat(outx_shuffled).detach().cpu().numpy().reshape(-1,n)
xnoisy_out_shuffled = torch.cat(outxnoisy_shuffled).detach().cpu().numpy().reshape(-1,n)
xrec_out_shuffled = torch.cat(outxrec_shuffled).detach().cpu().numpy().reshape(-1,n)
