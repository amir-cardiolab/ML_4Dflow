...
velocity_field = 'w'
Xw, mesh = read_velocity_data(input_dir, filename, reader, t_transient, t_end, velocity_field)
...
out_filename = 'batch1/N2V_L1_reconstruction_w'
print('Saving the noisy velocity field to ',out_filename)
meshNew = dsa.WrapDataObject(mesh)
mesh.GetCellData().RemoveArray('u')
mesh.GetCellData().RemoveArray('v')
mesh.GetCellData().RemoveArray('w')
for j in range(0,snapshots):
    meshNew.CellData.append(u_noisy[:,j], 'w')
    meshNew.CellData.append(N2Vrecu[:,j],'N2V_w')
